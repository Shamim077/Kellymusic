...
async def init():
    ...
    for all_module in ALL_MODULES:
        importlib.import_module("Kellymusic.plugins" + all_module)
    LOGGER("Kellymusic.plugins").info("Successfully Imported Modules...")
    await userbot.start()
    await Anony.start()
    try:
        await Anony.stream_call("https://te.legra.ph/file/29f784eb49d230ab62e9e.mp4")
    except NoActiveGroupCall:
        LOGGER("Kellymusic").error(
    ...
    await app.stop()
    await userbot.stop()
    LOGGER("Kellymusic").info("Stopping Kelly Music Bot...")
